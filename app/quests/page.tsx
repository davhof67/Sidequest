...
import { createQuest } from '@/actions/post-actions'
...
export default async function QuestsPage() {
    ...
    return (
        <main className="max-w-2xl mx-auto p-6 space-y-8">
            ...
            <form action={createQuest} className="space-y-4 p-4 border rounded-lg bg-gray-50">
            ...
            <div className="space-y-4">
                ...
                {quests.length === 0 ? (
                    ...
                    quests.map((quest) => (
                        <article key={quest.id} className="p-4 border rounded-lg shadow-sm">
                            <h3 className="text-lg font-bold">{quest.title}</h3>
                            <p className="text-gray-700 mt-1">{quest.description}</p>
                            <span className="text-xs text-gray-400 block mt-2">
                Von {quest.author.email} am {new Date(quest.createdAt).toLocaleDateString()}
              </span>
                        </article>
                    ))
                )}
